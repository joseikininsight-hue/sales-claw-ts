...
## 2.1.12 - 2026-08-14 — 送信フェーズの待ち時間短縮

- browser_wait_for の既定タイムアウトを 15秒→8秒 (送信完了画面の待ちが主用途。screenshot が送信記録なので短縮しても確実性は落ちない)。
- ペーストバナー fallback を 8秒→3.5秒 (v2.1.6 の ready マーカー強化で primary 検出が効くため)。
- 自動送信時のプロンプトに「送信後は snapshot を撮らず click→wait_for→screenshot(sent)→curl の最短手順で完了」を明記し LLM 往復を削減。

---

## 2.1.11 - 2026-08-14 — 並列トグルUI + 経路の可視化

- 設定タブに「複数社を並列ヘッドレスで処理」チェックボックスを追加 (ファイル直編集不要に。真因A=設定がサーバに届かない問題の恒久防止)。
- /api/ai-form-fill が並列/逐次どちらの経路を通ったか常に診断イベント+CLI Activity に記録。「並列ONのつもりが逐次だった」を即検知可能に。

---

## 2.1.10 - 2026-08-14 — 小バッチも全ワーカー同時稼働

- 並列グループサイズを適応化: 3社バッチが1グループ(=1プロセス逐次)に固まる問題を修正し、N社≦concurrency なら1社×Nプロセス同時に。
- 補足: phaseBParallel の外部ファイル編集はサーバのメモリ設定に反映されない (設定変更は必ずUI/API経由で行うこと)。

---

## 2.1.9 - 2026-08-14 — 並列ヘッドレスのワークキュー化 (大規模バッチ対応)

- 並列ディスパッチャを「concurrency個の巨大グループ事前分割」から「3社小グループのワークキュー」方式に変更。空いたプロセスが次グループを即座に取り、50-200社バッチでも連続消化できる (旧方式は50社が25社×2グループ+15分timeoutで全滅する構造欠陥)。
- slot timeout / プロセス死亡時に該当社の WebContentsView を必ず破棄 (入力途中の凍結タブ残骸を防止)。
- 並列 concurrency 既定を settings formFill.parallelism (1-3) に連動。

---

## 2.1.8 - 2026-08-14 — reCAPTCHA対策: 永続ブラウザプロファイル

- フォームセッションのパーティションを使い捨て (cookie/履歴ゼロ=毎回初訪問扱い) から persist:form-slot-0..2 の永続3スロット使い回しに変更。reCAPTCHA v3 スコア低下の主因を解消し、invisible reCAPTCHA で送信が通らない問題を改善。

---

## 2.1.7 - 2026-08-13 — 複数社バッチの並列ヘッドレス実行 (opt-in)

- formFill.phaseBParallel=true で複数社バッチを最大3プロセス同時の並列ヘッドレスへサーバ側委譲 (実測 ~4分/社 → 実効 ~1.5-2分/社)。既定 false。
- CLI issue 表示の切り取りを行末基準に変更 (プロンプト本文がエラー表示される誤解を解消)。

---

## 2.1.6 - 2026-08-13 — 起動〜記録パイプラインの信頼性強化

「起動したのにログが黙って消える」経路を実環境診断ログから洗い出して修正。

### 修正

- **action-log のプロセス間 lost update を解消**: Phase A 並列サブプロセスと
  dashboard 本体が同じ action-log.json を後勝ちで上書きし、エントリが静かに
  欠落していた。flush 前にディスク差分をマージし、別プロセスの書き込みと
  自プロセスの未 flush 分の両方を保持する。
- **PTY プロンプト書き込み失敗時の再キュー**: 旧仕様は write 失敗でプロンプト
  自体が消失し、PTY 復旧後もバッチが永久に再投入されなかった。queue 先頭へ
  戻して復旧後に自動再開する。
- **`/api/log-action` の 4xx 拒否を可視化**: 拒否 (422 等) が diagnostics にも
  ダッシュボードにも一切残らず「拒否されたが再送されなかった＝完全消失」が
  検知不能だった。全拒否を `log_action_rejected` 診断イベント + CLI Activity
  への SSE 警告として記録する。
- **Claude Code v2.x TUI の ready 検出**: プロンプト文字が `❯` (U+276F) に
  変わり旧マーカーが一度もマッチせず、常に 1.5 秒盲目タイマー頼みだった。
  現行 TUI のマーカーを追加し、早期誤検知防止に minReadyAge 1.2 秒を設定。
- Phase A の action-logger 読込失敗 (無言 no-op でログ全滅) と cli-logger の
  送達失敗 (ポート違い等) を stderr 警告で可視化。

### 検証

- unit: 73 files pass / 1 runtime-skip。クロスプロセスマージは実挙動検証済み。
- code-reviewer / security-reviewer 並列レビュー: CRITICAL/HIGH 指摘なし。

---

## 2.1.5 - 2026-06-13 — リファクタリング安全網と監査契約の復旧 (内部マイルストーン、2.1.6 に同梱)
