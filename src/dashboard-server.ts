...
function queueClaudeFormFillInManagedSession(companies, providerId = getManagedAiProvider(), options: Record<string, any> = {}) {
  ...
  const messageLines = isFirstBatchInSession ? [
    ...
    '',
    '★★ 推奨: sentMessage が複数行・引用符を含む長文の場合、curl の -d で shell escape ミスを起こしやすい。',
    '  → 本文を一時ファイル (例: $env:TEMP/body-<No>.txt or /tmp/body-<No>.txt) に BOM 無し UTF-8 で書き出し、',
    '    details に "sentMessageFile":"<absolute path>" を指定して渡す経路を使うこと。',
    '  → 例: {"no":<No>,...,"action":"awaiting_approval","details":{"sentMessageFile":"C:\\\\Users\\\\xxx\\\\AppData\\\\Local\\\\Temp\\\\body-<No>.txt","screenshot":"ss-<No>-input.png","tabKept":true}}',
    '  → サーバが %TEMP% / OS tmp dir / Sales Claw data dir 配下のパスから本文を読み込んで sentMessage として記録する。',
    '  → これで「メッセージが短すぎるエラー」「JSON 構文エラー」によるリトライループを完全に回避できる。',
    '',
