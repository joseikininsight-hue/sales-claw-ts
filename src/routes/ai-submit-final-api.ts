  ...
  async function handle(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      const prompt = [
        ...
        '【手順 (内蔵 MCP / 必須)】',
        navStep,
        '2. browser_snapshot({ sessionId }) でフォーム構造を取得 (戻り値の buttons 配列に送信/確認ボタン候補が selector 付きで入る)。',
        ...
        '   - 戻り値の validation.problems (必須未入力/未チェック/ラジオ未選択) が空になってから次へ進む。',
        `4. browser_take_screenshot({ sessionId, suffix: 'confirm' }) で送信前確認を撮影 (ss-${companyNo}-confirm.png)。`,
        '5. snapshot の buttons から「送信」「確認画面へ」「Submit」「確定」「Send」等のボタンを選び browser_click({ sessionId, selector }) で押す。',
